...
public class WaterNotificationData {
    ...
    public WaterNotificationData(Context context) {
        initFromLocal(context);
    }

    //for initialisation with fresh data from server
    public WaterNotificationData(Context context, DataSnapshot dataSnapshot) {
    ...
    public void updateLocalData(Context context) {
        ...
        if (hasChanged.contains(true)) {
            NotificationBroadcastReceiver.scheduleDay(context);
        }
    }
    ...
    //Getters & Setters are present beyond this point
