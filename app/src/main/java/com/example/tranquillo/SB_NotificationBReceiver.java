...
public class SB_NotificationBReceiver extends BroadcastReceiver {
    ...
    public static void scheduleDay(Context context) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                Intent intent = new Intent(context, SB_NotificationBReceiver.class);
                intent.putExtra(ALARM_TYPE, REMINDER);
                PendingIntent pendingIntent = PendingIntent.getBroadcast(context, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);
                AlarmManager alarmManager = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);

                ScreenBreakNotifData data = new ScreenBreakNotifData(context);
   ...
   private void publishNotification(Context context) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                final String CHANNEL_ID = "ScreenBreakNotification";

                NotificationManager notificationManager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);

                Notification.Builder builder = new Notification.Builder(context.getApplicationContext());
                builder.setContentTitle("Screen Break Notification");
                builder.setContentText("Take a break from Screen");
                builder.setSmallIcon(R.drawable.ic_baseline_notifications_24);
                builder.setGroup(CHANNEL_ID);
                builder.setVisibility(Notification.VISIBILITY_PUBLIC);

                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    NotificationChannel channel = new NotificationChannel(CHANNEL_ID, "Screen Break Notifications", NotificationManager.IMPORTANCE_HIGH);
    ...
    private void checkAndCancelFurtherAlarms(Context context) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                Intent intent = new Intent(context, SB_NotificationBReceiver.class);
